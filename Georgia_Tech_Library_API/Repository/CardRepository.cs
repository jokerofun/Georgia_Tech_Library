...
namespace Georgia_Tech_Library_API.Repository
{
    public class CardRepository : ICardRepository
    {
        ...
        public async void Insert(Card obj)
        {
            var sql = "insert into Card values (@CardNumber, @DateOfIssue, @ExpirationDay);";

            using var connection = _dbConnectionFactory.CreateSqlConnection();
            connection.Open();
            await connection.ExecuteAsync(sql, new { obj.CardNumber, obj.DateOfIssue, obj.ExpirationDay });
        }

        public async void Update(Card obj)
        {
            var sql = "update Card set @DateOfIssue, @ExpirationDay WHERE CardNumber = @CardNumber;";

            using var connection = _dbConnectionFactory.CreateSqlConnection();
            connection.Open();
            await connection.ExecuteAsync(sql, new { obj.DateOfIssue, obj.ExpirationDay, obj.CardNumber });
        }
    }
}
